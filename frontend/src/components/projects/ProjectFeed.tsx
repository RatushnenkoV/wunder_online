...
function PostBubble({
  ...
  if (post.is_deleted) {
    return (
      <div className={`flex gap-2 ${isOwn ? 'flex-row-reverse' : ''}`}>
        <div className="text-xs text-gray-400 italic py-1">сообщение удалено</div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex gap-2 group ${isOwn ? 'flex-row-reverse' : ''}`}>
      {/* Аватар */}
      <div className="w-8 h-8 rounded-full bg-blue-100 flex-shrink-0 flex items-center justify-center text-blue-700 font-bold text-xs select-none mt-1">
        {initials}
      </div>
      <div className={`flex flex-col max-w-[70%] ${isOwn ? 'items-end' : 'items-start'}`}>
        {!isOwn && (
          <span className="text-xs text-gray-500 mb-0.5 px-1">{name}</span>
        )}
        <div
      ...
      <div className="border-t border-gray-100 px-4 py-3 bg-white">
        ...
        <div className="flex gap-2 items-end relative">
          ...
          <textarea
            ...
            rows={1}
            className="flex-1 rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none max-h-32 overflow-y-auto"
            style={{ fieldSizing: 'content' } as React.CSSProperties}
          />
          <button
            onClick={sendPost}
            disabled={!input.trim() || sending}
            className="flex-shrink-0 p-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-40 transition-colors"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
