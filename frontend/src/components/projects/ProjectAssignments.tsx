...
function FileChip({ attachment }: { attachment: AssignmentAttachment | SubmissionFile }) {
  return (
    ...
    >
      ...
      <span className="text-gray-400 dark:text-slate-500">{formatFileSize(attachment.file_size)}</span>
    </a>
  );
}
...
function SubmissionStatus({ taskStatus }: { taskStatus: TaskStatus | null | undefined }) {
  ...
  return <span className="text-xs font-medium text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-900 px-2 py-0.5 rounded-full">Сдано</span>;
}

// ─── Event Timeline ───────────────────────────────────────────────────────────

const EVENT_CONFIG = {
  submitted: { label: 'Сдано', dot: 'bg-purple-500', text: 'text-purple-700', bg: 'bg-purple-50 border-purple-100' },
...
function EventTimeline({ events }: { events: SubmissionEvent[] }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs font-medium text-gray-400 dark:text-slate-500 uppercase tracking-wide">История</p>
      {events.map((ev, i) => {
        ...
        return (
          <div key={i} className={`flex gap-2.5 border rounded-lg px-3 py-2 ${c.bg}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{ev.author}</p>
              {ev.comment && (
                <p className="text-xs text-gray-700 dark:text-slate-300 mt-1 whitespace-pre-wrap">{ev.comment}</p>
              )}
            </div>
          </div>
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-2xl my-8">
        {/* Header */}
        <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-700 flex items-start justify-between">
          {editMode ? (
            <div className="flex-1 min-w-0 pr-4 space-y-2">
              <input
                ...
                className="w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-1.5 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-purple-500"
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
              ...
              <input
                ...
                className="rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
              <div className="flex gap-2 pt-1">
                ...
                <button
                  ...
                  className="bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-slate-600"
                >
                  Отмена
                </button>
              </div>
            ...
            <div className="flex-1 min-w-0 pr-4">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100">{assignment.title}</h2>
                {isTeacher && (
                  <button
                    onClick={() => setEditMode(true)}
                    className="text-gray-400 dark:text-slate-500 hover:text-purple-500 transition-colors flex-shrink-0"
          ...
          <button
            onClick={onClose}
            className="text-gray-400 dark:text-slate-500 hover:text-gray-600 flex-shrink-0"
          >
        ...
        <div className="p-6 space-y-6">
          ...
          {!editMode && assignment.description && (
            <p className="text-sm text-gray-700 dark:text-slate-300 whitespace-pre-wrap">{assignment.description}</p>
          )}
          ...
          {assignment.lesson_title && (
            <div className="flex items-center gap-2 bg-purple-50 border border-purple-100 rounded-lg px-3 py-2">
              ...
              <a href={`/lessons/editor/${assignment.lesson}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-purple-700 hover:text-purple-900">
                {assignment.lesson_title}
              </a>
            </div>
          )}
          ...
          {assignment.attachments.length > 0 && (
            <div>
              <p className="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-2">Материалы</p>
          ...
          {!isTeacher && (
            <div className="border border-gray-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-gray-700 dark:text-slate-300">Моя работа</p>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 text-sm text-gray-600 dark:text-slate-400 border border-gray-300 dark:border-slate-600 rounded-lg px-3 py-2 hover:bg-gray-50 dark:hover:bg-slate-800 disabled:opacity-50"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                  </svg>
                ...
                <button
                  ...
                  className="ml-auto bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50"
                >
          ...
          {isTeacher && (
            <div>
              ...
              {loadingSubmissions ? (
                <div className="text-sm text-gray-400 dark:text-slate-500">Загрузка...</div>
              ) : submissions.length === 0 ? (
                <div className="text-sm text-gray-400 dark:text-slate-500">Нет сдач</div>
              ) : (
                <div className="space-y-3">
                  {submissions.map(sub => (
                    <div key={sub.id} className="border border-gray-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
                      {/* Student info + status */}
                      ...
                      {/* Submission text */}
                      {sub.text && (
                        <p className="text-sm text-gray-700 dark:text-slate-300 whitespace-pre-wrap">{sub.text}</p>
                      )}
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  ...
                                  className="bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"
                                >
                                  Отмена
                                </button>
                              </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-lg my-8">
        <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Новое задание</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Срок сдачи (опционально)</label>
            <input
              type="date"
              value={dueDate}
              onChange={e => setDueDate(e.target.value)}
              className="rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
          </div>
          {pickerLessons.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Прикреплённый урок (опционально)</label>
              <select
                value={lessonId ?? ''}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Материалы</label>
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                disabled={uploadingFile}
...
function AssignmentCard({
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-5 py-4 cursor-pointer hover:shadow-sm transition-shadow"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100 truncate">{assignment.title}</h3>
          </div>
          {assignment.description && (
            <p className="text-xs text-gray-500 dark:text-slate-400 line-clamp-2 mb-2">{assignment.description}</p>
          )}
          {assignment.due_date && (
            <p className="text-xs text-gray-400 dark:text-slate-500">Срок: {formatDate(assignment.due_date)}</p>
          )}
        </div>
        <div className="flex-shrink-0 text-right">
          {isTeacher ? (
            <div className="flex flex-col items-end gap-1">
              ...
              <span className="text-xs text-gray-400 dark:text-slate-500">{assignment.submissions_count} сдач</span>
            </div>
          ) : (
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300">Задания</h3>
        {isTeacher && (
          <button
            onClick={() => setShowCreate(true)}
            className="flex items-center gap-1.5 text-sm text-purple-600 hover:bg-purple-50 px-3 py-1.5 rounded-lg transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 dark:text-slate-500 text-sm py-8">
          ...
          Нет заданий
          {isTeacher && (
            <button onClick={() => setShowCreate(true)} className="block mt-2 text-purple-600 text-sm hover:underline mx-auto">
