...
export default function NewDirectModal({ onClose, onOpened }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm mx-4 flex flex-col max-h-[70vh]">
        <div className="px-5 py-4 flex items-center justify-between" style={{ boxShadow: '0 1px 0 #f0f0f0' }}>
          <h2 className="text-lg font-semibold text-gray-800 dark:text-slate-200">Новое сообщение</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading && (
            <p className="text-center text-sm text-gray-400 dark:text-slate-500 py-6">Загрузка...</p>
          )}
          {!loading && displayed.length === 0 && (
            <p className="text-center text-sm text-gray-400 dark:text-slate-500 py-6">Никого не найдено</p>
          )}
          {displayed.map((u) => (
            <button
              key={u.id}
              onClick={() => open(u.id)}
              disabled={opening}
              className="w-full text-left flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors"
            >
              ...
              <div>
                <p className="text-sm font-medium text-gray-800 dark:text-slate-200">{u.display_name}</p>
                <p className="text-xs text-gray-400 dark:text-slate-500">{roleLabel(u)}</p>
              </div>
            </button>
          ))}
        </div>
