...
function IconDrag() {
  return <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-gray-300 dark:text-slate-600" fill="currentColor" viewBox="0 0 24 24"><path d="M8 6a2 2 0 100-4 2 2 0 000 4zm0 8a2 2 0 100-4 2 2 0 000 4zm0 8a2 2 0 100-4 2 2 0 000 4zm8-16a2 2 0 100-4 2 2 0 000 4zm0 8a2 2 0 100-4 2 2 0 000 4zm0 8a2 2 0 100-4 2 2 0 000 4z" /></svg>;
}
...
export default function SlideThumb({ slide, index, isSelected, isDragOver, onClick, onDelete, onDragStart, onDragOver, onDragLeave, onDrop, compact }: SlideThumbProps) {
  ...
  return (
    <div
      ...
      className={`group flex items-center gap-2 px-3 py-2.5 cursor-pointer transition-colors border-b border-gray-100 dark:border-slate-700 ${isSelected ? 'bg-purple-50' : 'hover:bg-gray-50 dark:hover:bg-slate-800'} ${isDragOver ? 'border-t-2 border-t-purple-400' : ''}`}
    >
      ...
      <span className="text-xs text-gray-400 dark:text-slate-500 w-4 text-center flex-shrink-0 font-medium">{index + 1}</span>
      <span className={`flex-1 text-xs truncate ${isSelected ? 'text-purple-700 font-medium' : 'text-gray-600 dark:text-slate-400'}`}>{label}</span>
      <button onClick={e => { e.stopPropagation(); onDelete(); }} className="flex-shrink-0 opacity-0 group-hover:opacity-100 p-1 text-gray-400 dark:text-slate-500 hover:text-red-500 transition-all rounded"><IconTrash /></button>
    </div>
  );
}
