...
export default function CuratorTab() {
  ...
  if (!classInfo) {
    return <div className="text-gray-400 text-sm py-8 text-center">Не удалось загрузить данные класса</div>;
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Класс {classInfo.class_name}</h2>
          <p className="text-sm text-gray-500">{classInfo.academic_year} учебный год</p>
        </div>
        <span className="text-sm text-gray-500">{classInfo.students.length} учеников</span>
      </div>
        ...
        <div className={isDesktop ? 'w-64 shrink-0' : 'w-full'}>
          <div className="bg-white rounded-lg shadow overflow-hidden">
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className={`text-sm font-medium truncate ${
                    isDesktop && selectedStudent?.id === student.id ? 'text-blue-700' : 'text-gray-800'
                  }`}>
            ...
            {classInfo.students.length === 0 && (
              <p className="text-center text-gray-400 py-8 text-sm">Ученики в классе не найдены</p>
            )}
          </div>
        </div>
          ...
          <div className="flex-1 min-w-0">
            {selectedStudent ? (
              ...
              <div className="flex items-center justify-center h-40 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                <p className="text-sm text-gray-400">Выберите ученика из списка слева</p>
              </div>
            )}
          </div>
        )}
