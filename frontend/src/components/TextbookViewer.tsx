...
export default function TextbookViewer({ title, fileUrl, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-950">
      ...
      {/* ── Тулбар ───────────────────────────────────────────────────── */}
      <div className="flex items-center gap-3 px-4 py-2.5 bg-gray-900 dark:bg-slate-900 border-b border-gray-800 flex-shrink-0 select-none">
        ...
        {numPages > 0 && (
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded text-gray-300 dark:text-slate-600 hover:bg-gray-700 disabled:opacity-30 transition-colors"
            ...
            <div className="flex items-center gap-1 text-sm text-gray-300 dark:text-slate-600">
              <input
                type="number"
                min={1}
                max={numPages}
                ...
                className="w-12 text-center bg-gray-700 dark:bg-slate-600 text-white rounded px-1 py-0.5 text-sm focus:outline-none focus:ring-1 focus:ring-purple-500 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
              />
              <span className="text-gray-500 dark:text-slate-400">/ {numPages}</span>
            </div>
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={zoomOut}
            disabled={scale <= 0.5}
            className="w-7 h-7 flex items-center justify-center rounded text-gray-300 dark:text-slate-600 hover:bg-gray-700 disabled:opacity-30 transition-colors text-lg font-light"
            title="Уменьшить"
          >−</button>
          <span className="text-xs text-gray-400 dark:text-slate-500 w-10 text-center">{Math.round(scale * 100)}%</span>
          <button
            onClick={zoomIn}
            disabled={scale >= 3}
            className="w-7 h-7 flex items-center justify-center rounded text-gray-300 dark:text-slate-600 hover:bg-gray-700 disabled:opacity-30 transition-colors text-lg font-light"
        ...
        <a
          ...
          className="w-7 h-7 flex items-center justify-center rounded text-gray-300 dark:text-slate-600 hover:bg-gray-700 transition-colors"
          title="Скачать"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <Document
          ...
          error={
            <div className="flex flex-col items-center justify-center h-64 gap-3 text-red-400">
              ...
              <a href={fileUrl} download className="text-purple-400 text-sm hover:underline">Скачать вместо этого</a>
            </div>
          }
        >
          <Page
      ...
      {numPages > 0 && (
        <div className="flex items-center justify-center gap-4 px-4 py-2.5 bg-gray-900 dark:bg-slate-900 border-t border-gray-800 flex-shrink-0 select-none">
          <button
            onClick={() => goToPage(1)}
            disabled={pageNumber <= 1}
            className="text-xs text-gray-400 dark:text-slate-500 hover:text-white disabled:opacity-30 transition-colors px-2 py-1 rounded hover:bg-gray-700"
          >
            В начало
          </button>
          <div className="flex items-center gap-1">
            {/* Миниатюрная полоска прогресса */}
            <div className="w-32 h-1 bg-gray-700 dark:bg-slate-600 rounded-full overflow-hidden">
              <div
